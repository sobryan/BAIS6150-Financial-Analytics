...
maxMeanV
for(muP in c(maxMeanV,.30,.27,.24,.21,
             .18,.15,.12,.09,.06,.03)) {
...
#unit test:
weightPortOOS(c('^GSPC'),252,1,c(1.0))
weightPortOOS(c('PCLN'),252,1,c(1.0))
weightPortOOS(c('^GSPC','PCLN'),252,2,c(.1,.9))
...
checkCovMat(cov_mat)
mufree <- 0
res    <- pruneBySharpe(prices,lab,meanv,sdevv,.0456)
prices <- res[[1]]
lab    <- res[[2]]
R   <- findR(prices)
res <- findCovMat(R)
...
checkCovMat(cov_mat)
mufree <- 0
res    <- pruneBySharpe(prices,lab,meanv,sdevv,.085)
prices <- res[[1]]
lab    <- res[[2]]
sum(is.na(prices[1,]))
R   <- findR(prices)
