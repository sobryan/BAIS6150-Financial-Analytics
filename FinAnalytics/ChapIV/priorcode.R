...
library(Quandl)
library(quantmod)
get.hist.quote <- function(lab,start,end,quote="Adj") {
  packVer <- packageVersion("quantmod")
  packVerLen <- nchar(packVer)
  packMajor <- as.numeric(substr(packVer,1,1))
  packMinor <- as.numeric(substr(packVer,3,5))
  if(packMajor >= 1.0 || packMinor >= 4.8) {
    #Use quantmod getSymbols() since it will quote equity ETFs:
    suppressWarnings(getSymbols(lab,
                                from = start, to = end))
    AdjClose <- 6
    sym <- lab
    #pv <- as.vector(GSPC[,AdjClose]) e.g.
    if(substr(sym,1,1) == '^') #e.g. "^GSPC"
      sym <- substr(sym,2,nchar(lab))
    pv <- as.vector(eval(parse(text=sym))[,AdjClose])
    
    pv <- c(pv,pv[length(pv)]) #for getSymbols() add 1 dup
    for(i in 1:length(pv)) #fix occasional missing price:
      if(is.na(pv[i]) && i > 0) pv[i] <- pv[i-1]
    return(pv)
  }
  #Use Quandl for S&P 500 but that will inc total return
  if(lab == "^GSPC" ) {
    QuandlCode <- 'BCIW/_SPXT'
    AdjClose <- 5
  } else { 
    #use Quandl for others:
    QuandlCode <- paste0('WIKI/',lab)
    AdjClose <- 12
  }
  Quandl.api_key("g-iR2rq2ev-eoqFeNNoW")
  rev(Quandl(QuandlCode,type="raw", 
             start_date=start,end_date=end)[,AdjClose])
}
...
APUT <- function(isTestElimSyms=FALSE) {
  ...
  p <- acquirePrices(p,l,len,D,D1,D2,dir,
       start="2010-02-18",end="2014-02-14",isSubDir=TRUE)
...
#unit test: 
library(tseries)
setwd(paste0(homeuser,"/FinAnalytics/MVO6/NYSE"))
get.hist.quote("BRK_B",start="2016-05-01",end="2017-05-01")
pdf <- getHistPrices(c('PCLN','MCD'),c(.5,.5),1*252,
